...
if args.generate_summary:
    ...
    prompt += "---\n"
    prompt += "write a short summary description paragraph for the above video on YouTube.\n"
...
if args.generate_chapters:
    ...
    for sentence in build_summary(trim=True):
        prompt += f"[{sentence['start_time']} - {sentence['end_time']}] {sentence['text']}\n"
    prompt += "---\n"
    prompt += "write up to 10 high-level chapters for the video on YouTube in the format: " + \
              "'MM:SS <chapter-title>.'\n"
